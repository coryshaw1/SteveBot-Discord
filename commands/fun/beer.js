...
module.exports = function (bot, data) {
    if (!data || !data.params || data.params.length < 1) { return getRandomBeer(bot, data); } else { return getSearchedBeer(bot, data); }
};
...
function messageBeer (beer, bot, data) {
    ...
    if (beer.style && beer.style.shortName && beer.abv) { beerInfo = beer.nameDisplay + ' (' + beer.style.shortName + ' ' + beer.abv + '%)'; } else { beerInfo = beer.nameDisplay; }

    if (beer.breweries && beer.breweries.length > 0) { beerInfo += ' - brewed by ' + beer.breweries[0].name; }
